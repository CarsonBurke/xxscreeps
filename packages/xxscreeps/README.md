...
The Screeps engine this repository uses as its reinforcement-learning environment; the
[project README](../../README.md) covers the RL stack that drives it. The engine and the
documentation below are upstream work from
[laverdet/xxscreeps](https://github.com/laverdet/xxscreeps) by Marcel Laverdet, so the
first person below is his. Changes made here for RL — a reproducible player realm, a
switch for NPC invasion waves, exported intent room-set keys — are listed in the project
README.
...
world directly in your browser: [@xxscreeps/client](../client/README.md).
...
autocomplete. If not you can read the [config schema](config/config.ts).
...
See [samples/bots/README.md](../../samples/bots/README.md). Open-source bots published for vanilla
servers also work as-is:
...
an optional `--json` envelope mode for tooling. See [cli/README.md](cli/README.md) for flags
...
```
git clone https://github.com/CarsonBurke/screeps-rl.git
cd screeps-rl
...
to be developed. The [architecture overview](../../docs/architecture.md) explains how the major
